...
module.exports = {
	...
	run(client, guildChannel) {
		...
		if (guildChannel.type == 'text') {
			fill = `nowy kanał tekstowy ${guildChannel}.`;
		}
		else if (guildChannel.type == 'news') {
			fill = `nowy kanał ogłoszeniowy ${guildChannel}`;
		}
		else if (guildChannel.type == 'voice') {
			fill = `nowy kanał głosowy ${guildChannel.name}.`;
		}
		else if (guildChannel.type == 'category') {
			fill = `nową kategorię ${guildChannel.name}`;
		}


		const channelCreateEmbed = new MessageEmbed()
			.setDescription(`➕ Utworzono ${fill}`)
		...
		db.get(query, [guildChannel.guild.id], (err, row) => {
			...
			if (!row) return;
			guildChannel.guild.channels.cache.get(row.channel_id).send(channelCreateEmbed);
		});
	},
};
